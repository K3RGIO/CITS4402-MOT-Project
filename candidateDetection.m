...
p = parser(fileLocation, nameTemplate, [10 20]) %frame range not specified because we want to load all images
...
% function ObjectDetectionButtonPushed(app, event)
    ...
    for i = p.frameRange(1)+1:p.frameRange(2)-1
        ...
        arr = cell(wholeBlockRows+1, wholeBlockCols+1); % creating empty array to input results into
        ...
        for row = 1:wholeBlockRows+1
            for col = 1:wholeBlockCols+1
                ...
                arr{row,col} = binary_12.*binary_13;
            end
